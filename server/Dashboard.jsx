...
const FlameIcon = ({ className, level = 'Spark', isLit = true }) => {
  ...
  if (!isLit) {
    fillGradient = 'url(#dormantGradient)';
    glowColor = 'rgba(148, 163, 184, 0.1)';
  } else if (level === 'Blaze') {
    fillGradient = 'url(#blazeGradient)';
    glowColor = 'rgba(20, 184, 166, 0.7)';
  } else if (level === 'Mythic Flame') {
    fillGradient = 'url(#mythicGradient)';
...
// # MAIN COMPONENT
export default function Dashboard({ userData, ingredientsData, questsData }) {
  
  // # DATABASE DATA CONTEXT
  const user = userData || { username: 'Koki', xp: 0, firewood: 0, currentStreak: 0, maxStreak: 0, streakThreatened: false, isFireLit: false };
  ...
  const getSimulatedNow = () => new Date();

  // # LINEAR DECAY MATHEMATICS
  const calculateIngredientHealth = (ingredient) => {
    if (ingredient.status !== 'active') return 0;
    const now = getSimulatedNow().getTime();
    const purchase = new Date(ingredient.purchase_date || ingredient.purchaseDate).getTime();
    const expiry = new Date(ingredient.expiry_date || ingredient.expiryDate).getTime();
  ...
  const flameLevel = useMemo(() => {
    const currentStr = user.currentStreak || user.current_streak || 0;
    if (currentStr >= 30) return 'Mythic Flame';
    if (currentStr >= 7) return 'Blaze';
    return 'Spark';
  }, [user]);

  const isFireLit = user.isFireLit || user.is_fire_lit;
  ...
  const handleAddIngredient = (e) => {
    ...
    router.post('/ingredients', {
      ...
      quantity: formQty,
      unit: formUnit,
      days_to_expiry: formDaysToExpiry
    }, {
      onSuccess: () => {
  ...
  const handleOpenEditModal = (ing) => {
    ...
    const now = getSimulatedNow().getTime();
    const expiry = new Date(ing.expiry_date || ing.expiryDate).getTime();
  ...
  const handleSaveEditIngredient = (e) => {
    e.preventDefault();
    router.put(`/ingredients/${editingIngredient.id}`, {
      name: editName,
      quantity: editQty,
      unit: editUnit,
      days_to_expiry: editDaysToExpiry
    }, {
      onSuccess: () => {
  ...
  const handleCookIngredient = (id) => {
    router.post(`/ingredients/${id}/cook`, {}, {
      onSuccess: () => {
  ...
  const handleWasteIngredient = (id) => {
    if(confirm('Tandai bahan ini telah membusuk dan dibuang?')) {
      router.post(`/ingredients/${id}/waste`, {}, {
        onSuccess: () => triggerToast('Bahan ditandai sebagai food waste.', 'error')
      });
    }
  };

  const handleAdjustQuantity = (id, direction) => {
    router.put(`/ingredients/${id}/adjust`, { direction }, {
      preserveScroll: true,
      onSuccess: () => triggerToast('Volume kuantitas stok disesuaikan.', 'success')
    });
  };

  const handleDeleteIngredient = (id) => {
    if(confirm('Hapus log bahan ini secara permanen dari server?')) {
      router.delete(`/ingredients/${id}`, {
        onSuccess: () => triggerToast('Data bahan dibersihkan.', 'neutral')
      });
    }
  };

  // # KITCHEN & SHOP ACTIONS
  const handleBuyFirewood = () => {
    router.post('/shop/buy-firewood', {}, {
      onSuccess: () => triggerToast('✨ Menukar 50 XP dengan 1 Kayu Bakar.', 'xp')
    });
  };

  const handleLightFireWithWood = () => {
    router.post('/kitchen/ignite-wood', {}, {
      onSuccess: () => triggerToast('🪵 Kayu dilempar! Api dapur berhasil dijaga.', 'success')
    });
  };

  const handleClaimQuest = (questType, id) => {
    router.post(`/quests/claim/${questType}`, { quest_id: id }, {
      onSuccess: () => triggerToast('🎉 Misi berhasil diklaim! Bonus XP masuk dompet.', 'xp')
    });
  };

  const simulateNextDay = () => {
    router.post('/simulation/next-day', {}, {
      onSuccess: () => triggerToast('⏩ Waktu melompat +1 Hari.', 'neutral')
    });
  };
    ...
    <div className="min-h-screen bg-stone-900 text-stone-100 font-sans selection:bg-emerald-600 selection:text-white">
      ...
      <header className="sticky top-0 z-40 backdrop-blur-md bg-stone-900/90 border-b border-emerald-900/30 px-4 py-3 md:px-8">
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row justify-between items-center gap-4">
          ...
          <div className="flex items-center gap-3 flex-wrap justify-center">
            ...
            <div className="bg-zinc-800/90 px-3 py-1.5 rounded-full border border-emerald-900/20 text-xs shadow-sm">
              <span className="text-emerald-400 font-black">🪵 {user.firewood || user.firewood_stock || 0} Kayu</span>
            </div>
            <div className={`flex items-center gap-2 px-4 py-1.5 rounded-full border transition-all duration-300 ${isFireLit ? 'bg-emerald-950/60 border-emerald-600/50 text-emerald-300 shadow-[0_0_10px_rgba(16,185,129,0.2)]' : 'bg-zinc-800 text-stone-400 border-zinc-700'}`}>
              ...
              <span className="text-sm font-black">{user.currentStreak || user.current_streak || 0} Hari Streak</span>
            </div>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto px-4 py-6 md:px-8 grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <section className="lg:col-span-7 flex flex-col gap-6">
            ...
            {['Segar', 'Waspada', 'Kritis', 'Busuk'].map((filter) => {
              ...
              return (
                <button
                  ...
                  className={`p-4 rounded-2xl border text-left transition-all duration-300 ${colors[filter]}`}
                >
          ...
          <div className="bg-gradient-to-r from-emerald-950/30 to-zinc-800/40 rounded-2xl p-5 border border-emerald-900/30 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
              <h2 className="text-lg font-black tracking-wide text-emerald-100 flex items-center gap-2">
                🍃 Inventaris Hijau Dapur
              </h2>
              <p className="text-xs text-stone-400">Pertahankan ekosistem bahan pangan bebas food-waste</p>
            </div>
            <button
              onClick={() => setIsAddModalOpen(true)}
              className="flex items-center gap-2 px-4 py-2.5 bg-gradient-to-r from-emerald-600 to-green-600 hover:from-emerald-500 hover:to-green-500 active:scale-95 text-white font-bold rounded-xl transition-all shadow-md text-sm w-full sm:w-auto justify-center"
            >
              ...
              {['Semua', 'Segar', 'Waspada', 'Kritis', 'Busuk', 'Dimasak'].map((filter) => (
                <button
                  ...
                  className={`px-4 py-2 rounded-xl text-xs font-bold whitespace-nowrap transition-all duration-200 ${
                    activeFilter === filter
                      ? 'bg-emerald-900/40 text-emerald-400 border border-emerald-600 shadow-sm'
                      : 'bg-zinc-800/80 text-stone-400 hover:bg-zinc-800 hover:text-stone-200'
                  }`}
                >
                  {filter}
                </button>
          ...
          <div className="flex flex-col gap-4">
            {filteredIngredients.length === 0 ? (
              ...
              filteredIngredients.map((ing) => {
                ...
                return (
                  <div key={ing.id} className="bg-zinc-800/80 border border-zinc-700/80 hover:border-emerald-800/50 rounded-2xl p-5 flex flex-col gap-4 transition-all shadow-sm relative overflow-hidden">
                    ...
                    {ing.status === 'active' && !isWasted && (
                      <div className="flex flex-col gap-1.5">
                        ...
                        <div className="flex justify-between text-[10px] text-stone-400 font-mono">
                          ...
                          <span>Sisa: {Math.max(0, Math.ceil((new Date(ing.expiry_date || ing.expiryDate).getTime() - getSimulatedNow().getTime()) / (1000 * 60 * 60 * 24)))} Hari</span>
                        </div>
                      </div>
                    )}
                    ...
                    <div className="flex justify-between items-center border-t border-zinc-700/60 pt-3">
                      ...
                      <div className="flex gap-2">
                        {ing.status === 'active' && !isWasted && (
                          <>
                            <button onClick={() => handleCookIngredient(ing.id)} className="px-3 py-1 bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold rounded-lg shadow transition-all">
                              🔥 Nyalakan Dapur
                            </button>
        ...
        <aside className="lg:col-span-5 flex flex-col gap-6">
          ...
          <div className="bg-gradient-to-b from-emerald-950/20 to-zinc-800/40 border border-emerald-900/30 rounded-3xl p-6 flex flex-col gap-5 text-center items-center shadow-lg relative overflow-hidden">
            ...
            <div className="flex flex-col items-center">
              ...
              <div className={`relative p-6 bg-zinc-950 rounded-full border transition-all duration-500 ${isFireLit ? 'border-emerald-500/60 shadow-[0_0_25px_rgba(16,185,129,0.25)] scale-105' : 'border-zinc-800'}`}>
                ...
                {isFireLit && (
                  <>
                    <span className="absolute text-sm animate-ping duration-[1500ms] top-2 right-2 opacity-75">🌱</span>
            ...
            <div className="mt-2 w-full z-10">
              <h3 className="text-3xl font-black tracking-tight text-white">{user.currentStreak || user.current_streak || 0} Hari Streak</h3>
            ...
            {/* Tingkatan Energi Progress Panel */}
            <div className="w-full bg-zinc-900/90 rounded-2xl p-4 text-left border border-zinc-700/80 z-10">
              ...
              <div className="grid grid-cols-3 gap-2 text-center text-[10px]">
                <div className={`p-2 rounded-lg border transition-all ${flameLevel === 'Spark' && isFireLit ? 'bg-emerald-950/60 border-emerald-500 text-emerald-300 font-bold scale-105' : 'bg-zinc-800 border-zinc-700/60 text-stone-500'}`}>
                  <span className="block">Spark</span>
                  <span>1-6 Hari</span>
                </div>
                <div className={`p-2 rounded-lg border transition-all ${flameLevel === 'Blaze' && isFireLit ? 'bg-teal-950/60 border-teal-500 text-teal-300 font-bold scale-105' : 'bg-zinc-800 border-zinc-700/60 text-stone-500'}`}>
                  <span className="block">Blaze</span>
                  <span>7-29 Hari</span>
                </div>
                <div className={`p-2 rounded-lg border transition-all ${flameLevel === 'Mythic Flame' && isFireLit ? 'bg-amber-950/60 border-amber-500 text-amber-300 font-bold scale-105' : 'bg-zinc-800 border-zinc-700/60 text-stone-500'}`}>
                  <span className="block">Mythic</span>
                  <span>30+ Hari</span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-zinc-900 border border-zinc-700/80 rounded-2xl p-4 flex justify-between items-center gap-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-stone-400">Gunakan saat tidak sempat memasak untuk menjaga stabilitas api.</p>
                </div>
              </div>
              <div className="text-right">
                <span className="block text-xs font-black text-lime-400 mb-1">50 XP</span>
                <button onClick={handleBuyFirewood} disabled={user.xp < 50} className={`px-3 py-1.5 text-[10px] font-bold rounded-lg transition-all ${user.xp >= 50 ? 'bg-emerald-600 text-white hover:bg-emerald-500 shadow-sm' : 'bg-zinc-800 text-stone-600 cursor-not-allowed'}`}>
                  Tukar
                </button>
              </div>
            </div>
          ...
          <div className="bg-zinc-900 border border-zinc-700/80 rounded-3xl p-5 flex flex-col gap-3">
            ...
            <button onClick={simulateNextDay} className="w-full py-2 bg-emerald-900/20 hover:bg-emerald-900/40 text-emerald-400 border border-emerald-800/40 text-xs font-black rounded-xl transition-all">
      ...
      {showToast && (
        <div className="fixed bottom-6 right-6 z-50 animate-bounce">
          <div className="px-5 py-3 rounded-2xl shadow-xl bg-zinc-900 border border-emerald-500 text-emerald-400 text-xs font-black shadow-emerald-950/50">
            🌿 {showToast.message}
          </div>
        </div>
      )}

      {/* MODAL WINDOWS (ADD / EDIT) */}
      {isAddModalOpen && (
          ...
          <div className="bg-zinc-900 border border-zinc-700 rounded-3xl w-full max-w-md p-6 shadow-2xl relative">
            ...
            <form onSubmit={handleAddIngredient} className="flex flex-col gap-4 text-xs">
              <div>
                ...
                <input type="text" placeholder="Misal: Sawi Organik, Daun Ketumbar..." value={formName} onChange={(e) => setFormName(e.target.value)} className="w-full bg-zinc-950 border border-zinc-700 rounded-xl px-4 py-2.5 text-stone-100 focus:outline-none focus:border-emerald-500" required />
              </div>
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <select value={formUnit} onChange={(e) => setFormUnit(e.target.value)} className="w-full bg-zinc-950 border border-zinc-700 rounded-xl px-4 py-2.5 text-stone-100 focus:outline-none focus:border-emerald-500">
                    <option value="gram">gram</option><option value="kilogram">kilogram</option><option value="ikat">ikat</option><option value="buah">buah</option>
                  </select>
                </div>
              </div>
              <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <select value={editUnit} onChange={(e) => setEditUnit(e.target.value)} className="w-full bg-zinc-950 border border-zinc-700 rounded-xl px-4 py-2.5 text-stone-100 focus:outline-none focus:border-emerald-500">
                    <option value="gram">gram</option><option value="kilogram">kilogram</option><option value="ikat">ikat</option>
                  </select>
                </div>
              </div>
              <div className="flex gap-2.5 mt-2">
